...
namespace Rito.Tests
{
    public class Test_ScreenShot : MonoBehaviour
    {
        ...
        ***********************************************************************/
        ...
        private bool _willShowScreen = false;
        #endregion
        /***********************************************************************
        *                               Fields & Properties
        ***********************************************************************/
        ...
        ***********************************************************************/
        ...
        {
#if UNITY_ANDROID //Permission.ExternalStorageWrite
        ...
        ***********************************************************************/
        ...
        private void OnPostRender()
        {
            if (_willShowScreen)
            {
                _willShowScreen = false;
                CaptureScreenAndShow(imageToShow);
            }
        ...
        /// <summary> 보여준 사진 저장하기 </summary>
        public void ShowScreenSave()
        {
            string totalPath = TotalPath; // 프로퍼티 참조 시 시간에 따라 이름이 결정되므로 캐싱
            // Texture2D screenTex = new Texture2D(Screen.width, Screen.height, TextureFormat.RGB24, false);
            Texture2D screenTex = imageToShow.sprite.texture;
            bool succeeded = true;
            try
            {
                // 폴더가 존재하지 않으면 새로 생성
